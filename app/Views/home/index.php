    ...
    <script>let CSRF={name:'<?= $csrfName ?>',hash:'<?= $csrfHash ?>'};</script>
        ...
        <div class="feed-post">
            ...
            <div class="feed-post-footer">
                ...
                <?php if (!empty($job->salary_min)): ?><?php $_sp=['annual'=>'/an','monthly'=>'/mois','daily'=>'/jour','hourly'=>'/h'];$_syms=['EUR'=>'€','USD'=>'$','GBP'=>'£'];$_sym=$_syms[$job->salary_currency??'']??($job->salary_currency??''); ?><span class="feed-badge salary"><i class="bi bi-cash me-1"></i><?= number_format($job->salary_min) ?><?= !empty($job->salary_max) ? '&ndash;'.number_format($job->salary_max) : '+' ?> <?= esc($_sym) ?><?= $_sp[$job->salary_period ?? 'annual'] ?? '/an' ?></span><?php endif; ?>
...
<div class="row g-3 mb-5">
    ...
    <div class="col-md-6 col-lg-4">
        <a href="<?= base_url('jobs/' . esc($job->slug)) ?>" class="land-job-card">
            ...
            <?php if (!empty($job->salary_min)): ?><?php $_sp=['annual'=>'/an','monthly'=>'/mois','daily'=>'/jour','hourly'=>'/h'];$_syms=['EUR'=>'€','USD'=>'$','GBP'=>'£'];$_sym=$_syms[$job->salary_currency??'']??($job->salary_currency??''); ?>
            <div class="mt-2 fw-semibold" style="color:#15803d;font-size:.82rem;">
                <i class="bi bi-cash me-1"></i>
                <?= number_format($job->salary_min) ?><?= !empty($job->salary_max) ? '&ndash;'.number_format($job->salary_max) : '+' ?>&nbsp;<?= esc($_sym) ?><?= $_sp[$job->salary_period ?? 'annual'] ?? '/an' ?>
            </div>
            <?php endif; ?>
        </a>
    </div>
